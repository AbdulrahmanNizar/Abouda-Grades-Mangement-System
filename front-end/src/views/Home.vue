<template>
  <div class="h-100 text-white">
    <nav
      class="navbar navbar-expand-lg bg-black bg-gradient text-white d-md-none d-block fixed-top"
    >
      <div class="container-fluid text-white">
        ...
        <div class="collapse navbar-collapse text-white" id="navbarSupportedContent">
          <ul class="navbar-nav me-auto mb-2 mb-lg-0 text-white">
            ...
            <li class="nav-item text-danger" style="cursor: pointer">Logout</li>
          </ul>
        </div>
      </div>
    </nav>
    ...
    <div class="vh-100 w-100 d-flex flex-row justify-content-start align-items-center m-0">
      ...
      >
        ...
        <div class="w-100 d-flex flex-column justify-content-center align-items-center mb-3">
          ...
          />
          <h6 class="text-center text-white mt-2">abouda</h6>
        </div>
        ...
        <div class="w-100 d-flex flex-column justify-content-center align-items-center">
          <hr class="w-100 text-white" />
          <button class="btn btn-outline-danger w-50" @click="logout">Logout</button>
        </div>
      </div>
      <div
        class="h-50 text-black d-flex flex-row justify-content-center align-items-center"
        style="width: 84%"
        id="dashboard"
      >
        adwada
...
const logout = async (): Promise<void> => {
  try {
    ...
    const response = await fetch(
      'http://127.0.0.1:3000/registration/logout/' + user_id.value,
      request_options,
    )
